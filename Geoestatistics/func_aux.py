...
def read_dados_exp(w_file):
    """Prepara X, Y e gr a partir dos arquivos."""
    df = read_data(w_file)
    # 1. Identificadores (gr) Indice 1 e 2. 
    gr = df.iloc[:, 1:3].to_numpy()
    # 2. Resposta (Y) : Indice 3
    Y = df.iloc[:, 3].to_numpy().reshape(-1, 1)
    # 3. Covariaveis (cov) : Indice 4 e 5
    cov = df.iloc[:,4:6].to_numpy()
    # 4. Construcao da matriz X (Intercepto + Covariaveis)
    n_linhas = cov.shape[0]
    coluna_uns = np.ones((n_linhas,1))
    X = np.hstack((coluna_uns,cov))
